...
class config:
    def __init__(self):
        ...
        if self.name == "artemis":
            ...
            self.gyro = Gyro(Port.S1, Direction.COUNTERCLOCKWISE)
            self.Llight = LightSensor(Port.S3)
            self.Rlight = LightSensor(Port.S4)

            self.lift = Forklift(self, self.RMmotor, 11, 40, 8)
            self.drive = DriveBase(self, self.Lmotor, self.Rmotor, self.gyro, self.runButton, 56, 104, Llight=self.Llight, Rlight=self.Rlight)
